...
// Dentro de la función `getProducts`, busca la sección donde se define `sql` y `countSql`.
// Comenta la consulta actual y reemplázala temporalmente con esta versión simplificada:
...
//   WHERE 1=1
// `
// ... (resto de filtros y GROUP BY)

// Reemplazar con (temporalmente):
let sql = `
  SELECT 
    p.id,
    p.name,
  ...
  WHERE 1=1
`
const params = [] // Asegúrate de que params esté vacío si no usas filtros

// Comentar toda la lógica de filtros (if active !== "all", if category, if search, etc.)
// Comentar la línea de GROUP BY:
...
// Comentar la consulta de conteo y reemplazarla:
// let countSql = `SELECT COUNT(*) as total FROM products p WHERE 1=1`
// const countParams = []
// ... (resto de filtros para conteo)

// Reemplazar con (temporalmente):
let countSql = `SELECT COUNT(*) as total FROM products`
const countParams = []

// Asegúrate de que la paginación siga funcionando con los nuevos `params` y `countParams`
// sql + ` LIMIT ? OFFSET ?`, [...params, limitNum, offset]
