...
try {
  ...
  // Explicitly create the final parameters array for the main query
  const finalProductQueryParams = [...params, limitNum, offset]

  // Ejecutar consultas en paralelo para mejor performance
  const [countResult, products] = await Promise.all([
    executeQuery(countSql, countParams),
    executeQuery(sql + ` LIMIT ? OFFSET ?`, finalProductQueryParams),
  ])
